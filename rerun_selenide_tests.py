...
def get_check_suite_url():
    if os.getenv('GITHUB_RUN_ATTEMPT') > '1':
        r = requests.get(f'https://api.github.com/repos/{os.getenv('GITHUB_REPOSITORY')}/actions/runs/{os.getenv('GITHUB_RUN_ID')}/attempts/{int(os.getenv('GITHUB_RUN_ATTEMPT')) - 1}',
                         headers={
